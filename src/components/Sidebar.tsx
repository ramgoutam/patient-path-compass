  ...
  return <div className={`fixed left-0 top-0 h-screen bg-slate-50 flex flex-col transition-all duration-300 ease-in-out z-10 ${collapsed ? 'w-16' : 'w-80'}`}>
      <div className="flex flex-col gap-4 flex-1 p-4 rounded-xl px-[12px] py-[21px] my-0 mx-0">
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className={`flex flex-col min-w-0 transition-all duration-300 ease-in-out ${collapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100 w-auto'}`}>
        ...
        <nav className="flex flex-col gap-2 flex-1">
          ...
          return <button key={item.href} onClick={() => onSectionChange(item.href)} className={cn("flex items-center text-sm font-medium transition-all duration-300 ease-in-out relative", collapsed ? "justify-center px-3 py-3 w-10 h-10 mx-auto" : "gap-3 px-3 py-2", isActive ? "text-slate-900" : "text-slate-700 hover:text-slate-900")} title={collapsed ? item.name : undefined}>
                {isActive && <div className={cn("absolute bg-slate-200 transition-all duration-300 ease-in-out", collapsed ? "inset-0 rounded-full" : "inset-0 rounded-full")} />}
      ...
      <div className="p-4 border-t border-slate-200 space-y-2">
        {/* Toggle button */}
        <button onClick={onToggleCollapse} className={cn("flex items-center text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-700 transition-all duration-300 ease-in-out rounded-md", collapsed ? "justify-center px-3 py-3 w-10 h-10 mx-auto" : "gap-3 px-3 py-2 w-full")} title={collapsed ? collapsed ? "Expand" : "Collapse" : undefined}>
        ...
        {/* Logout button */}
        <button onClick={handleLogout} className={cn("flex items-center text-sm font-medium text-red-600 hover:bg-red-50 hover:text-red-700 transition-all duration-300 ease-in-out rounded-md", collapsed ? "justify-center px-3 py-3 w-10 h-10 mx-auto" : "gap-3 px-3 py-2 w-full")} title={collapsed ? "Logout" : undefined}>
